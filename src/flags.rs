// Only call functions from within main.rsaaqaa for corresponding user interface
//
// FLAG TO FUNCTION
// -a = add();
// -b = backup();
...
pub fn add(vault: &String) {
    let mut user_input: String = "y".to_string();
        // Decrypt vault
        decrypt_vault(vault);
        // Loops while user_input = y or yes
        while user_input == "y" || user_input == "yes" {
            ...
            // Asks user if they would like to loop again, if n or no is entered, the loop is exited
...
pub fn delete(vault: &String) {
    let mut user_input: String = "y".to_string();
        ...
        while user_input == "y" || user_input == "yes" {
            ...
            // Handle error from account removal
            while error_handle != "ok" {
...
// USAGE (call after vault chose by user)
//
// change_account_username(vault_loaction);
...
// Gererate password and save to account if user chooses to
//
// USAGE (call after vault chose by user)
//
...
pub fn delete_vault_all_files(vault: &String) {
